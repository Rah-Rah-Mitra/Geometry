"""Artifact helpers for the ENEG notebook course."""
...
def find_book_root(start: Path | None = None) -> Path:
    """Find the nearest course root from a notebook or script working directory."""

    current = Path.cwd() if start is None else Path(start).resolve()
    for candidate in [current, *current.parents]:
        if (candidate / "00-book-index.ipynb").exists() and (candidate / "utils").exists():
            return candidate
    raise RuntimeError("Could not find Euclidean and Non-Euclidean Geometries book root")
