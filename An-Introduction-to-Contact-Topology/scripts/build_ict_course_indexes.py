"""Build book and unit indexes for the contact topology course."""
...
from ict_inventory import BACK_MATTER, ENTRIES, SOURCE_SPAN_NOTES
...
def build_book_index() -> str:
    lines = [
        "# An Introduction to Contact Topology - Standalone Notebook Course",
        "",
        "This course is an original executable notebook edition of Hansjorg Geiges's *An Introduction to Contact Topology*. It uses the local PDF only for source orientation and replaces passive reading with generated diagrams, symbolic checks, sampled flows, and computational labs.",
        "",
        "## Source Notes",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Validation",
            "",
            "Run the book-local source-map export, index builder, compile check, notebook audit, visual audit, limited execution, and `git diff --check` from the workspace root.",
        ]
    )
    return "\n".join(lines)
