"""Book-local artifact helpers for the Pressley course."""
...
def assert_artifact(path: str | Path, *, min_bytes: int = 1) -> Path:
    resolved = Path(path)
    if not resolved.exists():
        raise FileNotFoundError(resolved)
    if resolved.stat().st_size < min_bytes:
        raise ValueError(f"artifact is too small: {resolved} ({resolved.stat().st_size} bytes)")
    return resolved
