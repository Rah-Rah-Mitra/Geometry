"""Execute GDL course notebooks with nbclient."""
...
def notebook_paths(all_notebooks: bool, limit: int | None) -> list[Path]:
    ...
    if all_notebooks:
        paths.extend(inventory.index_path(entry) for entry in inventory.ENTRIES)
    paths.extend(inventory.canonical_path(entry) for entry in inventory.ENTRIES)
...
def main() -> None:
    ...
    parser.add_argument("--all", action="store_true", help="execute every index and canonical notebook")
