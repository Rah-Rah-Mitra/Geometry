"""Audit geometric group theory notebooks for standalone depth and structure."""
...
def main() -> None:
    ...
    failing = [
        ...
        or item["source_span_refs"] < 1
        or item["library_routing_refs"] < 1
        or item["stale_paths"]
        or item["generic_warnings"]
    ]
    ...
    if failing:
        ...
        for item in failing:
            print(
                ...
                f"final sanity refs={item['final_sanity_refs']}, source refs={item['source_span_refs']}, "
                f"library refs={item['library_routing_refs']}, stale_paths={item['stale_paths']}, "
                f"generic_warnings={item['generic_warnings']}"
            )
        raise SystemExit(1)
