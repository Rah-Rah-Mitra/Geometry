"""Build Euclid and Beyond book and unit indexes."""
...
from eab_inventory import BACK_MATTER, ENTRIES, SOURCE_SPAN_NOTES
...
def build_book_index() -> str:
    lines = [
        "# Geometry: Euclid and Beyond - Standalone Notebook Course",
        "",
        "This course is an original executable notebook edition of Robin Hartshorne's *Geometry: Euclid and Beyond*. It uses the local PDF only for source orientation and replaces passive reading with generated diagrams, plots, model checks, and computational labs.",
        "",
        "## Source Notes",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Validation",
            "",
            "Run the book-local bootstrap, index builder, notebook audit, visual audit, smoke execution, and `git diff --check` from the workspace root.",
        ]
    )
    return "\n".join(lines)
