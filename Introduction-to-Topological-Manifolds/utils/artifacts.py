"""Artifact helpers for the Introduction to Topological Manifolds course."""
...
def assert_artifacts(paths: Iterable[Path], *, min_bytes: int = 64) -> None:
    ...
    if tiny:
        messages.append("too small: " + ", ".join(tiny))
    if messages:
...
def display_artifact(path: Path, *, width: int = 760, height: int = 520) -> None:
    ...
    candidate = Path(path)
    try:
        display_path = candidate.resolve().relative_to(find_book_root().resolve()).as_posix()
    except ValueError:
    ...
    if suffix in {".png", ".jpg", ".jpeg"}:
        ...
        display(HTML(f'<iframe src="{display_path}" width="{width}" height="{height}"></iframe>'))
    elif suffix == ".json":
        display(Markdown(f"`{candidate.name}`"))
    else:
        display(Markdown(f"[{candidate.name}]({display_path})"))
