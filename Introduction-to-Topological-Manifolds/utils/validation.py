...
def canonical_notebooks(book_root: Path = BOOK_ROOT) -> list[Path]:
    ...
    return [
        p
        for p in sorted(book_root.rglob("*.ipynb"))
        if artifact_root not in p.parents and p.name not in IGNORED_NOTEBOOKS
    ]
...
def index_notebooks(book_root: Path = BOOK_ROOT) -> list[Path]:
    ...
    return [
        p
        for p in sorted(book_root.rglob("*.ipynb"))
        if artifact_root not in p.parents and p.name in IGNORED_NOTEBOOKS
    ]
...
def ensure_one_canonical_per_unit(book_root: Path = BOOK_ROOT) -> list[str]:
    ...
    for item in COURSE_MAP:
        ...
        if notebooks != [item["notebook"]]:
            findings.append(
                f"{item['folder_path']} canonical notebooks should be "
                f"{[item['notebook']]} but found {notebooks}"
            )
