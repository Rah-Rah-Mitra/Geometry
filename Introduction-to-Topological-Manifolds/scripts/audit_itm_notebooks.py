    ...
    for path in paths:
        for source in markdown_sources(path):
            ...
            if len(normalized.split()) >= 55:
                by_hash.setdefault(hashlib.sha256(normalized.encode()).hexdigest(), []).append(
                    relative(path)
                )
    return [
        {"finding": "repeated long markdown cell", "paths": sorted(set(v)), "sha256": k}
...
def main() -> None:
    ...
    findings.extend({"path": "", "finding": f} for f in ensure_one_canonical_per_unit(BOOK_ROOT))
    findings.extend(fingerprints(notebooks))
    report = {
