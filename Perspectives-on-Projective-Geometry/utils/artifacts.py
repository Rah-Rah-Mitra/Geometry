...
def artifact_path(root: str | Path, category: str, filename: str) -> Path:
    path = ensure_artifact_root(root) / slugify(category) / filename
...
def display_artifact(path: str | Path, width: int | str | None = None, height: int | None = None):
    ...
    if suffix in {".html", ".htm"}:
        return display(IFrame(src=book_relative(resolved), width=width or "100%", height=height or 440))
