...
def stats(path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "sanity": "final-sanity.json" in code,
...
def main() -> None:
    ...
    for chapter in inv.CHAPTERS:
        ...
        path = inv.canonical_notebook_path(chapter)
        if not path.exists():
        ...
        if not storyboard.exists():
            failures.append(f"{storyboard.relative_to(BOOK_ROOT)} missing")
        if item["crop"]:
    ...
    if failures:
        for failure in failures:
            print("FAIL:", failure)
        raise SystemExit(1)
    print("All canonical notebooks meet the configured standalone and structure checks.")
