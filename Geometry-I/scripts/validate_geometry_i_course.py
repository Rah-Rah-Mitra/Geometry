"""Execute Geometry I notebooks with nbclient."""

from __future__ import annotations

import argparse
import asyncio
import logging
import sys
from pathlib import Path

import nbformat
from nbclient import NotebookClient

BOOK_ROOT = Path(__file__).resolve().parents[1]
SCRIPT_ROOT = Path(__file__).resolve().parent
if str(BOOK_ROOT) not in sys.path:
    sys.path.insert(0, str(BOOK_ROOT))
if str(SCRIPT_ROOT) not in sys.path:
    sys.path.insert(0, str(SCRIPT_ROOT))

import geometry_i_inventory as inventory  # noqa: E402
from utils.validation import canonical_notebooks, index_notebooks, relative  # noqa: E402

if sys.platform.startswith("win"):
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())

logging.getLogger("tornado.general").setLevel(logging.CRITICAL)

SMOKE_NAMES = {
    "00-book-index.ipynb",
    "introduction.ipynb",
    "00-notation-and-background.ipynb",
    "01-group-actions-examples-and-applications.ipynb",
    "04-projective-spaces.ipynb",
    "08-euclidean-vector-spaces.ipynb",
    "11-convex-sets.ipynb",
}


def selected_paths(*, smoke: bool, all_notebooks: bool, limit: int | None) -> list[Path]:
    candidates = [BOOK_ROOT / "00-book-index.ipynb", *canonical_notebooks(BOOK_ROOT)]
    if all_notebooks:
        candidates = [*index_notebooks(BOOK_ROOT), *canonical_notebooks(BOOK_ROOT)]
    elif smoke:
        candidates = [path for path in candidates if path.name in SMOKE_NAMES]
    if limit is not None:
        candidates = candidates[:limit]
    return candidates


def execute(path: Path, timeout: int) -> None:
    nb = nbformat.read(path, as_version=4)
    client = NotebookClient(nb, timeout=timeout, kernel_name="python3", resources={"metadata": {"path": str(path.parent)}})
    client.execute()


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--smoke", action="store_true")
    parser.add_argument("--all", action="store_true")
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--timeout", type=int, default=300)
    args = parser.parse_args()

    paths = selected_paths(smoke=args.smoke, all_notebooks=args.all, limit=args.limit)
    if not paths:
        raise SystemExit("no notebooks selected")
    failures: list[tuple[Path, str]] = []
    for index, path in enumerate(paths, start=1):
        print(f"[{index}/{len(paths)}] {relative(path)}")
        try:
            execute(path, args.timeout)
        except Exception as exc:  # noqa: BLE001
            failures.append((path, repr(exc)))
    if failures:
        for path, error in failures:
            print(f"FAILED {relative(path)}: {error}", file=sys.stderr)
        raise SystemExit(1)
    expected = len(inventory.ENTRIES)
    print(f"Executed {len(paths)} notebooks successfully; course has {expected} canonical source units")


if __name__ == "__main__":
    main()
