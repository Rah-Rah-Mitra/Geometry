"""Artifact helpers for the Modern Robotics notebook course."""
...
def artifact_dir(topic: str, kind: str | None = None, root: str | Path = ARTIFACT_ROOT) -> Path:
    parts = [slugify(topic)]
    if kind:
...
def artifact_path(topic: str, kind: str | None, filename: str, root: str | Path = ARTIFACT_ROOT) -> Path:
    path = artifact_dir(topic, kind, root) / filename
...
def save_json(
    ...
    *,
    ...
    path = artifact_path(topic, kind, filename, root)
