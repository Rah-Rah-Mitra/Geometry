"""Audit generated visual artifacts for MFGDL."""
...
def image_stats(path: Path) -> dict[str, Any]:
    ...
    return {"path": rel(path), "width": rgb.width, "height": rgb.height, "bytes": path.stat().st_size, "sha256": digest, "max_channel_stddev": max(stat.stddev) if stat.stddev else 0.0}
...
def audit() -> dict[str, Any]:
    ...
    for entry in inventory.ENTRIES:
        ...
        topic_root = BOOK_ROOT / "artifacts" / str(entry["artifact"])
...
def main() -> None:
    ...
    if args.json:
        ...
        if report["findings"]:
            ...
            print("All MFGDL visual checks passed.")
