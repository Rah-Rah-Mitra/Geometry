...
def save_matplotlib(figure: Any, topic: str, filename: str, *, kind: str = "figures", dpi: int = 150, root: str | Path = ARTIFACT_ROOT, **kwargs: Any) -> Path:
    path = artifact_path(topic, kind, filename, root=root)
    figure.savefig(path, dpi=dpi, bbox_inches="tight", **kwargs)
    return path

def save_plotly_html(figure: Any, topic: str, filename: str, *, root: str | Path = ARTIFACT_ROOT, include_plotlyjs: str | bool = "cdn", full_html: bool = True, **kwargs: Any) -> Path:
...
def save_image(image: Any, topic: str, filename: str, *, kind: str = "figures", root: str | Path = ARTIFACT_ROOT) -> Path:
    ...
    if array.ndim == 2:
        PILImage.fromarray(array, mode="L").save(path)
    else:
