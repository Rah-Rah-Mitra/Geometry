"""Artifact helpers for the Hartshorne Algebraic Geometry course."""
...
def image_stats(path: str | Path) -> dict[str, Any]:
    ...
    try:
        relative = resolved.resolve().relative_to(BOOK_ROOT).as_posix()
    except ValueError:
...
def _effective_min_size(path: Path, min_size: int) -> int:
    if path.suffix.lower() in {".json", ".csv", ".txt", ".md"}:
        return min(min_size, 64)
    return min_size
...
def assert_artifacts(paths: Iterable[str | Path], *, min_size: int = 256) -> None:
    for item in paths:
        ...
        required_size = _effective_min_size(path, min_size)
