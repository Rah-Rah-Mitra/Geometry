"""Small SO(3)/SE(3) utilities for robotics notebooks."""
...
def twist_from_axis(axis: np.ndarray, point: np.ndarray, pitch: float = 0.0) -> np.ndarray:
    ...
    v = -np.cross(w, q) + pitch * w
    return np.r_[w, v]


def prismatic_twist(direction: np.ndarray) -> np.ndarray:
    d = np.asarray(direction, dtype=float).reshape(3)
    d = d / np.linalg.norm(d)
    return np.r_[np.zeros(3), d]


def instantaneous_power(wrench: np.ndarray, twist: np.ndarray) -> float:
    return float(np.dot(np.asarray(wrench, dtype=float).reshape(6), np.asarray(twist, dtype=float).reshape(6)))
