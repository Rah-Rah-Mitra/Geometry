"""Validation helpers for Geometry II scripts."""
...
CHAPTER_NUMBERS = tuple(range(12, 21))
...
def chapter_folders(book_root: Path = BOOK_ROOT) -> list[Path]:
    return sorted(path for path in book_root.iterdir() if path.is_dir() and path.name.startswith("chapter-"))
...
def artifact_topics() -> list[str]:
    return [f"chapter-{number:02d}" for number in CHAPTER_NUMBERS]
...
def ensure_one_canonical_per_chapter(book_root: Path = BOOK_ROOT) -> list[str]:
    ...
    for number in CHAPTER_NUMBERS:
        ...
        matches = [path for path in chapter_folders(book_root) if path.name.startswith(prefix)]
        if len(matches) != 1:
            findings.append(f"expected one folder matching {prefix}, found {len(matches)}")
            continue
        folder = matches[0]
