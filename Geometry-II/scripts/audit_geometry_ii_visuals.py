"""Audit Geometry II visual artifacts and notebook display calls."""
...
VISUAL_SAVE_CALLS = {"save_matplotlib", "save_plotly_html", "save_figure"}
...
def main() -> None:
    ...
    for topic in artifact_topics():
        ...
        htmls.extend(sorted(topic_root.rglob("*.html")) if topic_root.exists() else [])
        if not pngs and not htmls:
            findings.append({"check": "missing-topic-visual", "path": relative(topic_root, BOOK_ROOT)})
        for png in pngs:
    ...
    report = {
        "summary": {"notebook_count": len(notebook_stats), "png_count": len(images), "html_count": len(htmls), "finding_count": len(findings)},
        "findings": findings,
    ...
    if args.json:
        ...
        print(f"Audited {len(notebook_stats)} notebooks, {len(images)} PNGs, and {len(htmls)} HTML visuals")
        if findings:
            print(f"{len(findings)} finding(s):")
            for finding in findings:
                print(f"- {finding['check']}: {finding.get('path', finding.get('paths', ''))}")
        else:
            print("All Geometry II visual audit checks passed.")
    if findings:
        raise SystemExit(1)
